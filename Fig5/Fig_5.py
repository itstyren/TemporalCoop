...
conf.read('./Config_Fig3.ini', encoding='UTF-8')
...
def plot_param_as_function_compare_three_param(limit_iterator_list_1,
    ...
    config_src = 'Config_Fig3.ini'    # or 'Config_Fig5.ini'
    ...
    if mode == 'hpc':
        ...
        # Run simulation
        strategy_destination = run_simulation_show_destination_compare_three_param(
            graph, limit_iterator_list_1, limit_iterator_list_2,
            unlimit_iterator_list,
    ...
    if mode == 'hpc_to_local':
        ...
        if list_name_2.split('_')[0] == 'r':
            labels = [
                '%s = ' % (list_name_2.split('_')[0]) + str(value)
                for value in limit_iterator_list_2
            ]
        else:
            labels = [
                r'$\%s$ = ' % (list_name_2.split('_')[0]) + str(value)
                for value in limit_iterator_list_2
            ]
        ...
        fig_d_name = os.path.join(last_runtime, 'd-' + run_time + '.eps')
...
def run_simulation_show_destination_compare_three_param(
        graph, limit_iterator_list_1,
        limit_iterator_list_2,
        unlimit_iterator_list,
    ...
    for limit_iterator_1_index, limit_iterator_1_value in enumerate(
        ...
        param_dict = {param_name_1: limit_iterator_1_value}

        unlimit_iterator_list_tuple = []
        for limit_iterator_index, limit_iterator_value in enumerate(limit_iterator_list_2):
            for unlimit_iterator_index_1, unlimit_iterator_value_2 in enumerate(unlimit_iterator_list):
                unlimit_iterator_list_tuple.append((limit_iterator_value, unlimit_iterator_value_2))
        random.shuffle(unlimit_iterator_list_tuple)

        param_name_tuple = (param_name_2, param_name_3)
        ...
        for u_i_1 in limit_iterator_list_2:
            single_strategy_destination = []
            for u_i_2 in unlimit_iterator_list:
...
def main():
    if switch_fuc == 'd':
        plot_param_as_function_compare_three_param(
           sigma_iterator_list, r_iterator_list, p_iterator_list,
